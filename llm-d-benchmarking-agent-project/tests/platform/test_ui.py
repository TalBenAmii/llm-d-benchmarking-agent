...
the wiring against drift; the visual behaviour itself is exercised by hand via ``app/ui/preview.html``.
...
def test_preview_harness_exists_and_is_self_contained():
    """app/ui/preview.html drives the renderers with fixtures and no backend, for hand verification.
...
def test_markdown_table_rendering_is_wired():
    ...
    app/ui/preview.html (which seeds an assistant bubble containing a pipe-table)."""
    js = _ui("app.js")
    css = _ui("styles.css")
...
def test_share_a_chat_via_link():
    ...
    # Minting shows the SAME-ORIGIN link (absolute when SHARE_BASE_URL is set) — no publish step.
    assert "function setShareUrl" in js and "setShareUrl(localUrl)" in js
    assert "/publish" not in js                                 # the gist-publish path is gone
...
# running the JS. The visual behaviour is exercised by hand via ``app/ui/preview.html``.
...
# and app.js stays brace-balanced. Behaviour itself is exercised by hand via app/ui/preview.html.
