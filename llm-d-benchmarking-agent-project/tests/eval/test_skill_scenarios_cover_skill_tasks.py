...
scenario, and each scenario's skill_dir must point at that task's real SKILL.md
path. Fully hermetic (reads only the local knowledge yaml + the eval's SCENARIOS);
needs no sibling repos.
"""
...
def test_scenarios_cover_exactly_the_skill_tasks():
    """The eval scenarios and the key_docs.yaml *_skill tasks are the same set."""
    yaml_skill_tasks = {e["task"] for e in _key_docs() if e["task"].endswith("_skill")}
    scenario_keys = {s.key for s in SCENARIOS}
    assert scenario_keys == yaml_skill_tasks, (
        f"scenario/key_docs drift: only-scenario={scenario_keys - yaml_skill_tasks}, "
        f"only-yaml={yaml_skill_tasks - scenario_keys}"
    )


def test_scenario_keys_match_shared_skill_tasks_table():
    """The eval scenarios and the shared SKILL_TASKS table agree on the task set."""
    assert {s.key for s in SCENARIOS} == set(SKILL_TASKS)


def test_each_scenario_skill_dir_has_matching_skill_md_entry():
    """Each scenario.skill_dir matches a key_docs.yaml SKILL.md path for that task."""
    docs = _key_docs()
    for s in SCENARIOS:
        ...
        assert all(e["path"].startswith(s.skill_dir) for e in skill_md), (
            f"{s.key} SKILL.md path(s) {[e['path'] for e in skill_md]} not under {s.skill_dir}"
        )


def test_shared_table_dirs_match_scenario_dirs():
    """SKILL_TASKS and the eval scenarios agree on each operation's skill dir."""
    by_key = {s.key: s.skill_dir for s in SCENARIOS}
