"""Product-boundary guard — the debug-only ``testing/`` harness must never ship.

``testing/local-cluster/`` holds local mock-cluster scaffolding (multi-node kind, kwok
...
def test_dockerfile_copies_only_product_dirs():
    ...
    assert not leaked, (
        ...
        "Debug-only scaffolding (testing/) must never be baked into the product image."
    )


def test_dockerignore_excludes_testing():
    patterns = {
        ln.strip().rstrip("/")
    ...
    assert "testing" in patterns, (
        ".dockerignore must exclude `testing/` so the mock-cluster harness never enters "
        "the build context."
    )


def test_app_does_not_import_testing():
    pat = re.compile(r"^\s*(?:from|import)\s+testing\b")
    offenders = [
    ...
    assert not offenders, (
        f"product code imports the debug-only testing harness: {offenders}. "
        "app/ must never depend on testing/."
    )
