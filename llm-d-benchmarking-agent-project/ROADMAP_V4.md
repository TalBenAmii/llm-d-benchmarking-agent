...
## Phase 59 — Model-load readiness gate: poll `/v1/models` vs `/health` with stuck-pod load-timing diagnostics — TODO
...
## Phase 60 — Infra precondition gate: K8s server version + vLLM/NIXL image minimums + sidecar gotcha — TODO
...
## Phase 61 — Right-size the harness launcher CPU request for small/Kind clusters (`LLMDBENCH_HARNESS_CPU_NR`) — TODO
...
## Phase 62 — Gated-model access pre-flight (check_model_access / GatedStatus before standup) — TODO
...
## Phase 63 — Accelerator + CPU-inferencing precondition advisor (GPU-advertised? DRA vs device-plugin? 64c/64GB-per-replica?) — TODO
...
## Phase 64 — Provider-aware precondition pack (oc-vs-kubectl, GPU taints/tolerations, GMP/known-issues) — TODO
...
## Phase 65 — Gateway-mode readiness gate (Gateway PROGRAMMED + InferencePool Accepted/ResolvedRefs) — TODO
...
## Phase 66 — EPP HTTP-header decoder (interpret 429s + `x-llm-d-request-dropped-reason`) — TODO
