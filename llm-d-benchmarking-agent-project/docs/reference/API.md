...
| `orchestrate_sweep` | approve | `namespace`, `treatments` (each `{name, spec?, harness?, workload?, command?, cpu?, memory?}`), `spec`/`harness`/`workload` (defaults), `image`, `service_account`, `cpu`, `memory`, `scheduling`, `active_deadline_seconds`, `max_parallel`, `max_attempts`, `poll_interval`, `max_wait`, `sweep_id`, `checkpoint`, `require_ready_endpoint` | Run N DoE treatments as parallel Kubernetes Jobs under a `max_parallel` cap against one stood-up stack; per-treatment retry/dead-letter. Progress is checkpointed to a cluster ConfigMap: a re-call with the returned `sweep_id` + same treatments resumes (completed treatments skipped). Parallel counterpart to `execute_llmdbenchmark(subcommand="experiment")`. Needs an orchestrator image. `knowledge/orchestrator.md`. |
