...
|---|---|---|
| End-to-end flow: probe → plan → standup → smoketest → run → parse report → summarize → teardown | `app/agent/engine.py`, `app/tools/*` | 🔵 In the chat: *"benchmark a small chat model on CPU"* and approve the plan (exercised in a real session). |
...
| **Steering** (Claude-Code style): type a message WHILE the agent is working; it is queued and the running turn picks it up at its next step (no concurrent turn, no "please wait"). Also covers typing instead of approving at a gate (declines + steers) | `app/agent/engine.py` (`steer()` injects into the live SDK turn; `ctx.steer_messages` is the between-turns fallback), `app/main.py` (queues mid-turn `user_message` + backstop), `app/ui/app.js` (composer stays usable mid-turn) | 🔵 Start a turn, then send another message before it finishes; the agent folds it in. ⚪ `tests/agent/test_ws.py::test_ws_typing_while_thinking_steers_the_same_turn`, `tests/agent/test_sdk_engine.py` (steer battery), `tests/platform/test_concurrency.py::test_second_message_to_running_session_is_queued_as_steer`. |
...
| **Simulate Mode** (`SIMULATE=1`): walk the whole flow; read-only commands run for real, mutations are approval-gated then no-op, synthetic report | `app/config.py`, `app/tools/command_exec.py` + `app/tools/run/shell.py` (caller-gate), `app/agent/prompt.py` (SIMULATE_NOTE) | 🔵 Set `SIMULATE=1`, run a benchmark in chat: read-only probes/greps return real output (genuine context), every mutating command still raises its Approve/Reject card (SIMULATE previews the mutation, it does not waive the guardrail) and is then a no-op, a synthetic report is produced, no cluster touched. ⚪ `tests/tools/test_simulate.py`. |
