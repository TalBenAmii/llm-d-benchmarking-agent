...
Import `reproducibility`; two `_DESCRIPTIONS` entries (cue `read_knowledge('reproducibility')` + the dry-run-first sequence); two `ToolSpec` rows. (Tool count 28 → 30; update count refs in `CLAUDE.md:99`, `app/tools/CLAUDE.md:1`, `docs/API.md`, `FEATURES.md` — grep first, some are test-locked.)
...
- **Tool-count doc drift.** Several docs hard-code "28 tools"; grep + update so doc-consistency tests don't fail.
