...
- **2026-06-20: Claude Code hook teardown (Claude Code = Opus 4.8).** Removed the entire `.claude/`
  ...
  not version-controlled, so recreate on fresh clone). Net: zero Claude Code hooks.
...
- **2026-06-28: `run-llm-d-benchmark` skill rewritten (dropped `run_only.sh`); all 3 read-only repos verified current.**
  ...
  `llm-d-skills`@`5a14639` all 0/0 vs their `origin/main`. The skill's "v0.8.0" label just runs ahead of the
...
- **2026-06-28: `llm-d-skills` promoted to REQUIRED; knowledge adapters dedup'd to defer to it (reverses 4eb3323).**
  ...
  property (its 2 call sites, `repos.py` and `knowledge_access.py`, point at `repo_paths`). Net: `/readyz` now 503s
  ...
  **Knowledge dedup:** the 5 skill-adapter files were already mostly delta; removed the 3 short skill-step *recap*
  ...
  CPU-sim caveats, welllit map) is untouched. `sweep_playbook.md` + `author_spec_workload.md` were ~95% delta already, so left as-is.
...
- **2026-07-05: model ids gained a 4th home: `scripts/install/setup-claude-plan.sh`'s interactive menu.** The new
