...
Python is mechanism; judgment lives in the LLM + `knowledge/`. Do not put decision logic
(which spec/harness/workload, what flags, how to read a result, what to do about a fault) in
Python `if/elif` branches. Put the facts and reasoning in an editable `knowledge/*.md` file
and let the LLM reason over it. The Python layer only: serves the UI, runs the agent loop,
...
- A fake LLM provider: drives the agent loop deterministically (scripted tool calls), so no
...
absent, like `helm`/`promtool`, is the one acceptable skip, because the hermetic structural
test already covers the contract.)
...
sim, so it is gated on `LLMD_SIM_INTEGRATION=1` and the sim being locatable; it skips cleanly
by default and never hangs reaching a server that isn't there. Its wiring is still covered
hermetically: a sim-shaped BR v0.2 fixture (built from the repo's own example) is driven
through the real analyze/compare tools in the default suite, so the integration logic is
tested even when the sim is absent. See `knowledge/sim_integration.md`.
...
2. Write the handler. Add a function in a module under `app/tools/` taking
   ...
   `ctx.run_command` (which go through the allowlist + runner). Return a JSON-serializable
   dict of facts: no judgment, no prose verdicts the LLM should be forming.
...
4. Allow any new commands as data. If the tool needs a command not yet permitted, add it to
   ...
   approval-gated) and any value constraints / `timeout_s` / `quota`. Never add a Python
   branch.
...
phases are tracked in `FEATURES.md` (the DEFERRED phases), owned by the integrator). When
implementing one:
...
- No new runtime dependency without an explicit reason. The dependency surface is
...
- `mypy app`: meaningful but not `--strict` over the whole tree (that would demand a
  ...
  SDKs have `arg-type`/`call-overload` relaxed at that boundary only
  (`[[tool.mypy.overrides]]`). Prefer a precise annotation or a `TypeGuard` over a blanket
  `type: ignore`; targeted ignores must carry an error code.
