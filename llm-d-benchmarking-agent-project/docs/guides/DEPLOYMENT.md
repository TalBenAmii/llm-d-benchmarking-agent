...
### Prerequisites
- `uv` (the Python package/venv manager). `scripts/install/install_local.sh` / `scripts/run.sh` auto-bootstrap it
...
- A Claude subscription login for the `claude-agent-sdk` provider (the only supported one):
  the local `claude` CLI logged in, or a `claude setup-token` token in
  `CLAUDE_CODE_OAUTH_TOKEN`. Without it the server still boots and the deterministic
...
this to keep the agent next to the workloads it benchmarks, or to expose it to a team. This is
...
(plus `--oauth-token "$CLAUDE_CODE_OAUTH_TOKEN"`). Untested
today. Full operator runbook, including auth and RBAC:
**[`CLUSTER_SERVICE_DEPLOY.md`](CLUSTER_SERVICE_DEPLOY.md)**.

### Manual (build + raw Helm)

`install_service.sh` wraps these; run them by hand only for an air-gapped deploy:
...
helm install bench-agent deploy/helm/llm-d-benchmarking-agent \
  ...
  --set secret.claudeCodeOauthToken=$CLAUDE_CODE_OAUTH_TOKEN
```
...
| `config.llmProvider` / `config.agentSdkModel` | `claude-agent-sdk` / `claude-sonnet-5` | Provider (only `claude-agent-sdk` is supported) + model. |
...
| `secret.create` / `secret.existingSecret` / `secret.hfToken` | `true` / `""` / … | Chart-managed Secret, or point at a pre-existing one (recommended for real deploys). |
