...
> wide-EP, autoscaling); those need at least 2 GPUs to mean anything. On one card those stay
...
- writes the scenario plus a companion `<name>.spec.yaml` into the session workspace and
  returns its `spec_path`, which is what you feed to `--spec`.
...
- **Qwen2.5 is ungated, so no `HF_TOKEN` needed.** If you choose a gated model, set `HF_TOKEN` in
...
3. **Drive the real GPU run.** The agent defaults to the `cicd/kind` quickstart, so be explicit
   ...
   3. *"Author a single-GPU scenario for `Qwen/Qwen2.5-0.5B-Instruct`"*, giving it the override
...
A genuine end-to-end test of the multi-GPU features needs a cluster with at least 2 GPUs;
point the agent at one with `KUBECONFIG=…` and the same flow applies.
