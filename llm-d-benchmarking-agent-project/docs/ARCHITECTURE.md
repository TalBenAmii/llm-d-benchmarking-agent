...
The llm-d Benchmarking Agent is a **conversational agent + benchmark orchestrator +
...
boundaries, and the two invariants that make the whole thing safe and reliable.

## The two governing principles

Everything below is a consequence of two rules (see [`CLAUDE.md`](../CLAUDE.md)):
...
```
   ...
   │                                      ▼                                 │
   │            tool registry (app/tools/registry.py) — 32 tools            │
...
- **Connection handling** — resume a saved chat via `/ws?session=<id>`; replay history +
...
- `loop.py` — the control loop: assemble the system prompt → call the LLM → for each
  tool call, validate args and dispatch through the gate → stream results back → feed tool
  results to the model → repeat until the model stops calling tools (bounded by
  `MAX_STEPS`). One tool can never crash the loop; rejections and errors are returned to the
  model so it can replan.
- `prompt.py` — `build_system_prompt()` = a fixed `ROLE` + `HARD_RULES` + **every** file
  ...
  the "thick agent" lives: changing the agent's behavior is editing `knowledge/`, not code.
...
`registry.py` maps each tool name to a Pydantic input model + a handler.
...
through. The 32 tools and their schemas are catalogued in [`API.md`](API.md).
...
- `controller.py` — `BenchmarkOrchestrator`: `submit` (write manifest → `kubectl apply`),
...
plus weights + activation + KV-cache arithmetic. Pure math + diagnostics in `planner.py`;
...
5. A handler that runs a command goes through `ToolContext.run_command` →
   ...
   command is re-validated — defense in depth — and run, emitting the `command` event for
   what *truly* ran).
...
- **Backend ↔ child processes:** argv lists with `shell=False`; the subprocess environment
  is scrubbed so the LLM API key / HF token never reach a child (the HF token is passed only
