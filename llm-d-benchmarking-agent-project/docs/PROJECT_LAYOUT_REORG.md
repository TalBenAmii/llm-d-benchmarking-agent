...
```
**Touches:** every `docs/<X>.md` path reference — root + project `CLAUDE.md` maps (10+ refs), the
...
- `read_knowledge("name")` resolves by **stem** (`_match_knowledge_basename`), so name-based lookups
  survive a move **iff** the file enumeration recurses. Basenames must stay **globally unique**.
...
```
**Touches:** the `bridges/` scripts are invoked through the allowlist — moving them edits
...
flattens the surface and `tool_loader.py` already groups tools by phase logically. **Recommend skipping
or doing last.**
