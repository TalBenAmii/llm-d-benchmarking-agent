...
| HuggingFace token / gated-model auth | docs/standup.md#use; config/README.md#huggingface-configuration | LLMDBENCH_HF_TOKEN, huggingface.token, huggingface.enabled | Y | off | 🟡 | HF token kept backend-only/scrubbed (`app/config.py:child_env`); `check_capacity` now also runs a read-only **gated-model access pre-flight** via the repo's own `check_model_access`/`GatedStatus`, surfacing token-free `{gated, authorized, reason}` at the plan gate (Phase 62 ✅, `app/capacity/planner.py:merge_gated_access`, `knowledge/capacity.md`, `tests/test_capacity_gated.py`). Remaining gap: the agent does not yet provision the cluster HF secret for gated standups. Fix → ROADMAP_V4 Phase 30. |
