...
| `POST` | `/api/share/{token}/publish` | **Publish a public link** without exposing the agent: render the snapshot's `page.html` and upload it as a **secret** (unlisted) GitHub gist via the user's `gh` CLI. Returns `{public_url, fallback_url, gist_id, reused}` (`public_url` is a `gist.githack.com` render URL; `fallback_url` is the htmlpreview proxy). Owner-only (auth-gated — it creates content on the user's account, so it is *not* a public GET). `404` if the token isn't a live share; **`503` with a `reason`** (`gh-missing`/`gh-failed`) when `gh` is absent or fails, so the UI can fall back to the same-origin link. Idempotent per token (reuses a recorded gist). This is what the 🔗 dialog calls so the **public** link is shown by default. |
| `DELETE` | `/api/share/{token}` | Revoke a share link: delete its snapshot **and**, if one was published, its secret gist (best-effort — a `gh`/network failure still revokes the snapshot). Returns `{deleted, token, gist_revoked}`. Owner-only (auth-gated); `404` if already gone. |

> **Share-link auth.** Only the `GET` viewer routes (`/share/{token}`, `/api/share/{token}`) bypass the optional Bearer auth (the token is the bearer secret); minting (`POST`), **publishing** (`POST .../publish`), and revoking (`DELETE`) stay auth-gated. The snapshot exposes only the transcript the owner chose to share — never the session list, a live session, or secrets.
>
> **Sharing with someone off-host.** Two ways, depending on whether you want to expose the running app:
> - **Static-file export (no exposure) — recommended, and the default.** A shared chat is just a self-contained `.html` (the SPA + frozen snapshot inlined). Host it anywhere and the agent is never reachable.
>   - **Automatic, in-app:** clicking 🔗 **Share** mints the snapshot *and* publishes it (`POST .../publish` → a **secret GitHub gist**, unlisted; the gist id is the credential, like the share token), so the dialog shows the public link by default. **Delete link** revokes the gist too. Needs `gh` (authenticated); if it's missing the dialog falls back to the same-origin link and says so.
>   - **From a terminal:** `scripts/publish_shared_chat.sh <token-or-link>` does the same upload (`--revoke <token>` deletes it; `--dry-run` previews) — it shares the gist description + `<workspace>/shares/<token>.gist` mapping with the in-app path, so either can revoke the other's gists.
...
| `run_resilience_drill` | read-only* | `namespace`, `spec`/`harness`/`workload`/`image`, `chaos_plan`, `max_attempts`, `prove_restart`, `slo_budget_s` | **Opt-in chaos / resilience drill.** Injects faults via a `KubeClient` decorator that rewrites cluster reads → they flow through the **unmodified** classify → retry/dead-letter → reconstruct path, producing a genuine **resilience report** (per-fault injected vs classified vs recovery; a restart-durability proof — a fresh orchestrator resumes a partial sweep from the cluster checkpoint with 0 duplicate Jobs; SLO met/missed). **Double-gated:** refuses unless `CHAOS_ENABLED=true` **and** this named tool is invoked (never reachable from `orchestrate_benchmark_run`). *Runs against an in-process cluster — it issues no real `kubectl`, so it auto-runs. WHICH faults to inject + how to read the verdict is `knowledge/resilience.md`. |
