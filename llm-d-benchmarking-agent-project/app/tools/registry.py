...
_DESCRIPTIONS = {
    ...
    "provision_hf_secret": (
        ...
        "an input here, never in argv/events/logs. Two triggers: (1) after a check_capacity "
        "GATED+UNAUTHORIZED verdict whose reason says NO token is configured cluster-side, OR (2) "
        "when the user DIRECTLY asks to set up their HF token Secret for a gated model — CALL it "
        "then (calling it IS the proposal; the Approve card is the gate), do NOT first force a "
        "failed capacity check and do NOT hand-roll it via run_shell/run_setup. Still NEVER "
    ...
    "execute_llmdbenchmark": (
        ...
        "Every other option's exact CLI mapping, valid subcommands, and knowledge pointer is "
        "documented on the field itself (`models` for a non-default model, `store` for the "
        "team-shared results store, `flags` for every other CLI knob) — consult THAT field's "
        "description before relying on it; do not act from memory."
    ),
...
def build_registry() -> dict[str, ToolSpec]:
    ...
    specs = [
        ...
        ToolSpec("provision_hf_secret", _DESCRIPTIONS["provision_hf_secret"], ProvisionHfSecretInput, repos.provision_hf_secret),
        ...
        ToolSpec("compare_harness_runs", _DESCRIPTIONS["compare_harness_runs"], CompareHarnessRunsInput, compare.compare_harness_runs),
        ...
        ToolSpec("observe_run_metrics", _DESCRIPTIONS["observe_run_metrics"], ObserveRunMetricsInput, manage_runs.observe_run_metrics),
