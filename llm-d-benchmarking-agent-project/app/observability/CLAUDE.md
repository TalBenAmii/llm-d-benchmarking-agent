...
- **`instrument.REGISTRY` is the process-wide singleton** the `/metrics` endpoint renders. Tests must isolate
...
- **The dependency-free claim is load-bearing** — `metrics.py` / `logging.py` are hand-rolled (`JsonFormatter`,
  ...
  all read — keep them aligned. `logctx.bind` restores prior values exactly by token (nesting/concurrency safe).
- **`resource_poller` is self-limiting:** no-op when `ctx.emit is None` or `ctx.settings.simulate`; after
  ...
  match the same literal. It reuses the private `app.tools.observe._parse_top_table` — keep it stable.
...
## Key files
- `metrics.py` — `MetricsRegistry` / `Counter` / `Gauge` / `Histogram` + `render_prometheus`.
- `instrument.py` — metric defs + `record_*` helpers; owns `REGISTRY`; `bind_registry` / `use_registry`.
- `logging.py` (`setup_logging`, `JsonFormatter`) · `logctx.py` (`bind`, `new_corr_id`, `LOG_CONTEXT_FIELDS`).
