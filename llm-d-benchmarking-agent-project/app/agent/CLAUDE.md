...
`build_system_prompt()` (`prompt.py:143-157`) must return a **byte-identical** prefix across
...
+ `catalog_brief_message` / `prompt.py:160-168`), precisely so they don't mutate the cached
