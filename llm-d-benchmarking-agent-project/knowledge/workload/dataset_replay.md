...
## `conversation_replay` load semantics → its own guide

`conversation_replay` profiles (inference-perf `agentic_code_generation.yaml` etc.) have subtle
load semantics users routinely mis-model — think-time that HOLDS the concurrency slot, built-in
conversation recycling, and `num_requests`-based run length (not `request_timeout`). Before
answering ANY "N users" / run-duration / think-time question about them,
`read_knowledge('conversation_replay')` — the numbers are counter-intuitive and easy to get wrong.
