...
**Never seed history/trends with user-asserted numbers** — the `results_interpretation.md`
**§ "Honesty floor" (rule 1)** applied to the store: only a validated report
(`locate_and_parse_report` → `analyze_results` this session) may become a baseline / stored record /
trend point. If `result_history` returns **empty**, that empty result IS the answer — do NOT take
the user's recalled numbers as a baseline, render a PASS table from them, or seed the trend store;
say the history is empty (no validated baseline) and offer to **re-run that scenario** to establish
a machine-validated one before trending against it.
