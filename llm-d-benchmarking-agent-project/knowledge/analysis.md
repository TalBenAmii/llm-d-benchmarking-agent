...
This is the analyze-path face of the `results_interpretation.md` **§ "Honesty floor" (rule 2)** —
apply it verbatim: an absent metric is "not available", mark that dimension **"inconclusive"**
(never PASS/FAIL), and never estimate/interpolate/extrapolate the missing percentile (no p99 from
p90/p50, no "p99 ≥ p90 ⇒ ~240 ms"). A definitive ✅/❌ on an unmeasured metric is wrong even when
the margin looks generous. Analyze-path specifics: on a SIMULATE report (only
...
`analyze_results`/`compare_reports` operate on **validated Benchmark Reports** only — the
`results_interpretation.md` **§ "Honesty floor" (rule 1, incl. the verbal-disclaimer clause)**
applied to the analyzer: refuse SLO scoring, goodput, a Pareto/frontier, or any statistic (mean,
t-test, CI, LaTeX table) over numbers the user **pasted, typed, or recalled** or declared invalid;
**decline the specific result** they intend to use and require a validated report (re-run the
scenario) first.
