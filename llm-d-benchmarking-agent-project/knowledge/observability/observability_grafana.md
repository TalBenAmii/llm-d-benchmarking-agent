...
> Extracted from `knowledge/observability.md` (the §2 live-view choice + embedding) — read this when
...
- **Grafana — the richer, recommended view.** The llm-d observability dashboard (the upstream
  ...
  CAN stand this stack up for the user — never refuse to stand up Grafana, do NOT treat it as
  "their problem to solve" or claim you can "only advise". The upstream recipe is one approval-gated
  `run_shell` away, exactly like the metrics-server install and the kind-cluster create: read the
  exact commands from the upstream `install-prometheus-grafana.sh` / observability `setup.md`
  (`knowledge/useful_repo_docs.md` → `read_repo_doc`), then run them with `run_shell` (mutating →
  it raises the Approve card). Offer it the same way you offer the metrics-server install. The ONE
  piece that is genuinely the user's, not yours, is the backend env var `GRAFANA_DASHBOARD_URL` —
  you have no env/secret-write tool, so THEY set it (pointing at their Grafana); it controls only
  the in-panel embed (below), not whether the stack exists. `probe_environment` reports
...
- **metrics-server — the convenient alternative.** Live **CPU/memory only** (no GPU, no latency, no
  ...
  (`install_metrics_server.sh`, per observability.md §2) and it lights up the in-panel sparklines immediately. It is
  the right answer when the user just wants a quick "is anything melting?" view and has no Grafana
  stack.

Position Grafana as the fuller picture and metrics-server as the quick fallback. The two live-views
are independent — the Grafana embed works even when metrics-server is absent, and vice-versa (see
below for the embed).
...
they set **`GRAFANA_DASHBOARD_URL`** to that dashboard's URL. When set, an
...
`GRAFANA_DASHBOARD_URL` and I'll show it in the run panel.
