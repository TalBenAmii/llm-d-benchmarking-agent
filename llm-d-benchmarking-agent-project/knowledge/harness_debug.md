...
A normal `run` deploys the harness launcher and then **runs the benchmark load**. Debug mode
(`-d`/`--debug`, upstream env `LLMDBENCH_DEBUG`) deploys the **same harness pods but runs
`sleep infinity` instead of the load** — the pods come up, stay up, and do nothing, so a human
can `kubectl exec`/`oc exec` in and run commands by hand to diagnose why a real run is failing.

Set it by passing `flags={'debug': True}` on an `execute_llmdbenchmark(subcommand='run', …)`
**or** `subcommand='experiment'`. The tool emits a bare short `-d` after the subcommand.

## Run/experiment ONLY — and NEVER teardown

`-d`/`--debug` is **only** valid on `run` and `experiment` upstream. The SAME short `-d` means
something completely different — and **destructive** — on `teardown`:
...
Debug mode is **not** read-only: it creates real harness pods (they just sleep instead of
loading). A `run … -d` (or `experiment … -d`) is **mutating** and **requires approval**, exactly
like a normal run — it is *not* a collect-only/auto-run flag like `skip`/`-z`.

## WHEN to use it — your judgment

Reach for `debug: True` when a run is failing/misbehaving in a way you cannot diagnose from
logs/reports alone and you need a **live, stable pod to inspect by hand**:

- **The harness pod keeps crashing before you can look at it** — `sleep infinity` keeps it
  Running so a human can exec in and inspect filesystem, mounted dataset, env vars, network
  reachability to the model endpoint, installed tooling, etc.
- **You suspect an environment/wiring problem** (dataset volume didn't mount, wrong endpoint
  URL, missing token/secret, DNS to the service fails) and want to reproduce the harness's own
  view interactively (e.g. `curl` the endpoint from inside the pod).
- **You want to hand the user a pod to experiment in** before committing to a full real run.
...
step**; **the agent never drives it**:
...
4. When done, tear the stack/pods down (a normal, separate teardown).
