export const meta = {
  ...
  description: 'Read EVERY .md documentation file in both upstream repos (llm-d AND llm-d-benchmark), judge each one for usefulness when building THIS agent project, and emit a curated, annotated doc index: docs/USEFUL_REPO_DOCS.md (the human-readable catalog — a "start here" quicklist, per-repo/per-topic tables of path + what-it-covers + why-useful-for-us + reference-points, an API/feature reference-points section, an external-links section, and a low/skip appendix so NO doc is silently dropped) plus knowledge/useful_repo_docs.md (a compact runtime-readable pointer the agent auto-discovers). Fan-out doc clusters -> read+tier -> synthesize+commit -> adversarial verify (every file accounted for, named examples present, paths valid) with one bounded fix loop. Docs-only: writes/commits on branch docs/repo-docs-index off main (NEVER main; no push, no merge). Re-runnable when the upstream docs change.',
