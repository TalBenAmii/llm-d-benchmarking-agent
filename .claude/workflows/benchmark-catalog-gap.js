export const meta = {
  ...
  description: 'Cross-reference every llm-d-benchmark feature doc against this agent\'s coverage (tools + knowledge/ + FEATURES.md) and emit docs/BENCHMARK_FEATURE_COVERAGE.md (✅/🟡/⬜ catalog) + ROADMAP_V4.md (gap roadmap) + a knowledge pointer. Fan-out → extract → cross-ref → 3-lens skeptic → synthesize → verify. Docs-only on branch docs/benchmark-feature-catalog off main (no push/merge). Re-runnable.',
