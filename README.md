...
if one isn't running, runs the benchmark, and explains the results. It drives the real
`llmdbenchmark` CLI inside a strict security sandbox, and nothing changes your system without
your approval.
...
cross-session history and trends, shareable HTML reports, Prometheus/Grafana observability, a
...
> **Proof of concept, laptop-first.** The supported, exercised path today is running the
> assistant as a service on a local `kind` cluster on your laptop (the `./install.sh` path
> below). Running it against a real (remote / multi-node / GPU) cluster is not yet tested,
> but it uses the same service deploy, so the delta should be small: point `kubectl` at your
> real cluster and pull the image from a registry instead of `kind load`. See
...
That single run auto-installs anything missing (docker/kind/kubectl/helm) via `sudo`. It prompts
for your password, and if Docker was newly installed it stops once so you can log out/in
(docker-group activation) and re-run. On first run it also offers to wire your Claude
subscription (when the `claude` CLI is present) so chat works; accept, or skip and it still
deploys with health green (chat stays disabled until you add a token/key). It then builds the
image, creates the `kind` cluster, deploys via Helm, verifies `/healthz` and `/readyz`, leaves
the service running, and opens the UI in your browser.
...
[kind](https://kind.sigs.k8s.io/) cluster with a simulated inference engine. No GPU, no model
download. Open the UI and type:
...
Tell it your SLOs up front ("p99 TTFT under 500 ms, at least 1000 tok/s") and it turns a wall
...
It also extracts richer signals when the harness emits them (KV-cache hit rate, schedule delay,
GPU utilization) and reports `None` rather than guessing when a harness doesn't.
...
Read-only commands run for real so it gathers genuine context; mutating actions are announced
but no-opped to synthetic success, and a synthetic report is produced. The best way to watch a
guide end-to-end without touching a cluster.
...
```bash
cd llm-d-benchmarking-agent-project
make validate        # deterministic and hermetic: no API key, Docker, kind, or repos needed
